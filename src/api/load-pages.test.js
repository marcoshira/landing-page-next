...
describe('load-pages', () => {
  beforeEach(() => {
    ...
    mockJson = jest
      .fn()
      .mockResolvedValue(Promise.resolve({ data: 'mockData' }));
  ...
  it('should call fetch and mapData with correct values', async () => {
    ...
    expect(mapDataModule.mapData).toHaveBeenCalledWith('mockData');
  ...
  it('should call fetch with correct slug', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      config.url + '/api/pages?filters[slug][$eq]=atenotestando',
    );
  });
});
